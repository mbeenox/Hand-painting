...
# ---------------------------------------------------------------------------
# Local full backend: prefer the repo copy (lockstep with api/), fall back
# to the cv2 wheel's data dir (present locally, stripped on Vercel).
_CASCADE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             "..", "api", "haarcascade_frontalface_default.xml")
...
async def process_image(
    ...
    focus: str = Query("none", pattern="^(none|face)$"),
    span: int = Query(1, ge=1, le=2),
):
    ...
    # Optional face focus (camera flow): blur the background behind
    # detected faces so the stroke budget goes to the subject. No-op when
    # no face is found.
    n_faces = 0
    face_mask = None
    if focus == "face":
    ...
    # span=2 also SEES fainter edges (P88 vs P92); base map anchors "100%".
    edges = detect_edges(img, 85.0 if span == 2 else 92.0)  # Step A
    ...
    if mode == "trace":
        ...
        if span == 2:
            # 200%-completeness tier (see api/index.py for full rationale).
        ...
        chains = order_chains_in_passes(chains, rng)
